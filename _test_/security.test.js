...
describe('Защита и безопасность', () => {
    ...
    it('Должен отклонить POST без CSRF-токена (в тестовой среде пропускается)', async () => {
        ...
        // В тестовой среде CSRF отключен, поэтому запрос не блокируется и перенаправляет
